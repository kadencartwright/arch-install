...
- virtiofs repo sharing so the repo can be cloned to `/root/arch-install`

Manual takeover occurs once the repo clone to `/root/arch-install` is complete.
...
```bash
scripts/vm-libvirt-create.sh \
  --name arch-install-test \
  --iso "$HOME/Downloads/archlinux-2026.02.01-x86_64.iso" \
  --disk ./.vm/arch-install-test.qcow2 \
  --disk-size 120 \
  --memory 8192 \
  --cpus 4 \
  --repo "$(pwd)"
...
Open `virt-manager`, select `arch-install-test`, and connect to the SPICE console.
Boot into the Arch live environment.

## 3) Mount shared repo and clone to `/root/arch-install`
