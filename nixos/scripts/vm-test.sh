...
KEEP_TMP=0
TMP_DIR=""
WORK_DIR="${VM_TEST_WORK_DIR:-${HOME}/.cache/arch-install/nixos-vm-test}"
LOCAL_STORE=0
STORE_ROOT=""

usage() {
    cat <<'USAGE'
Usage: nixos/scripts/vm-test.sh [options] [mode]
...
Options:
  ...
  --no-copy           Evaluate the nixos/ directory directly instead of a temp copy
